            ...
            if($this->input->post()){
                ...
                if($this->form_validation->run()){
                    ...
                    $this->load->model('product_model');
                     foreach ($carts as $rows){
                        $product_if = $this->product_model->get_info($rows['id']);
                        //pre($product_if);
                        $data = array();
                        $data['buyed'] = intval($product_if->buyed) + intval($rows['qty']);
                        $this->product_model->update($product_if->id_product, $data);
                     }
                    ...
                    if($payment == 'offline'){
                        ...
                        $this->session->set_flashdata('message', 'Bạn đã đặt hàng thành công, chúng tôi sẽ kiểm tra và gửi hàng cho bạn. Cám ơn bạn đã lựa chọn chúng tôi!');
                        redirect();
